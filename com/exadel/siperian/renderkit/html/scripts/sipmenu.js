...
if(!RichFaces.SipMenu) RichFaces.SipMenu = {};

/**
 * Fixes IE bug with incorrect layer width when set to auto
 * @param layer
 */
RichFaces.SipMenu.fitLayerToContent = function(layer) {
    if (!RichFaces.SipMenu.Layers.IE)
			return;
...
RichFaces.SipMenu.removePx = function(e) {
	if ((e+"").indexOf("px")!=-1)
...
RichFaces.SipMenu.Layers = {
	...
	LMPopUpL: function(menuName, visibleFlag, event) {
		...
		RichFaces.SipMenu.fitLayerToContent(menu);
...
//			Element.hide(tmpLayer);
	...
	setLMTO: function (ratio) {
		...
		if (this.useTimeouts) {
			clearTimeout(this.timeoutFlag);
			this.timeoutFlag = setTimeout('RichFaces.SipMenu.Layers.shutdown()', ratio);
		}
	},
	...
	showMenuLayer: function (layerId, e, delay){
		this.clearPopUpTO();
		this.showTimeOutFlag = setTimeout(new RichFaces.SipMenu.DelayedPopUp(layerId, e, function(){this.layerId = null;}.bind(this)).show, delay);
		this.layerId = layerId;
	},
	showDropDownLayer: function (layerId, parentId, e, delay){
		this.clearPopUpTO();
		var menu = new RichFaces.SipMenu.DelayedDropDown(layerId, parentId, e);
		if (menu.show) {
...
if (window.attachEvent) {
    window.attachEvent("onunload", function() {
    	var layers = RichFaces.SipMenu.Layers;
    	layers.destroy();
	});
}
 ...
 */
RichFaces.SipMenu.getWindowElement = function() {
	return (document.documentElement || document.body);
}

RichFaces.SipMenu.getWindowDimensions = function() {
	var x,y;
...
RichFaces.SipMenu.getWindowScrollOffset = function() {
	var x,y;
...
RichFaces.SipMenu.getPageDimensions = function() {
	var x,y;
...
RichFaces.SipMenu.DelayedContextMenu = function(layer, e) {
    if (!e) {
        e = window.event;
    }
	//Event.stop(e);
    ...
    this.layer = $(layer);
    this.show = function() {
		RichFaces.SipMenu.Layers.shutdown();
		...
		RichFaces.SipMenu.Layers.LMPopUp(this.layer.id, false,e);
        RichFaces.SipMenu.Layers.clearLMTO();
    }.bind(this);
}
 ...
 */
RichFaces.SipMenu.DelayedDropDown = function(layer, elementId, e) {
	if (!e) {
		e = window.event;
	}
	...
	this.show = function() {
		...
		var winOffset	= RichFaces.SipMenu.getWindowScrollOffset();
		var win			= RichFaces.SipMenu.getWindowDimensions();
		var pageDims	= RichFaces.SipMenu.getPageDimensions();
...
//		}
		...
		var options = RichFaces.SipMenu.Layers.layers[this.layer.id].options;
		...
		RichFaces.SipMenu.Layers.LMPopUp(this.layer.id, false);
		RichFaces.SipMenu.Layers.clearLMTO();
	}.bind(this);
}

RichFaces.SipMenu.DelayedPopUp = function(layer, e) {
	if (!e) {
		e = window.event;
	}
    ...
    this.show = function() {
        if (!RichFaces.SipMenu.Layers.isVisible(this.layer) &&
        	RichFaces.SipMenu.Layers.isVisible(RichFaces.SipMenu.Layers.father[this.layer.id])) {
    	    ...
    	    RichFaces.SipMenu.Layers.LMPopUp(this.layer, false);
        }
    }.bind(this);
}

RichFaces.SipMenu.DelayedPopUp.prototype.reposition = function() {
	var windowShift = RichFaces.SipMenu.getWindowScrollOffset();
	var body = RichFaces.SipMenu.getWindowDimensions();
    var windowHeight = body.height;
    var windowWidth = body.width;
    var scrolls = {top:0, left:0};
    var screenOffset = Position.positionedOffset(this.element);
    var leftPx = RichFaces.SipMenu.removePx(this.element.parentNode.parentNode.style.left);
    var topPx = RichFaces.SipMenu.removePx(this.element.parentNode.parentNode.style.top);
...
/*    if (layerLeft + layerdim.width >= windowWidth) {
        layerLeft = left - layerdim.width + RichFaces.SipMenu.Layers.shadowWidth;
    }
    ...
    } else layertop = top - RichFaces.SipMenu.Layers.CornerRadius;
 ...
 */
RichFaces.SipMenu.selectOpen = false;
RichFaces.SipMenu.MouseIn = false;


RichFaces.SipMenu.Layer = Class.create();
RichFaces.SipMenu.Layer.prototype = {
	initialize: function(id,delay, hideDelay, selectedClass, onEvt){
		RichFaces.SipMenu.Layers.listl.push(id);
   		this.id = id;
   		this.layer = $(id);
   		this.level = 0;
   		this.delay = delay;
   		this.onEvt = onEvt ? this.eventJsToPrototype(onEvt) : onEvt;
        ...
        RichFaces.SipMenu.fitLayerToContent(this.layer);
        this.items = new Array();
   		RichFaces.SipMenu.Layers.layers[id] = this;
        ...
        this.mouseover =
	       function(e){
	             RichFaces.SipMenu.MouseIn=true;
		         RichFaces.SipMenu.Layers.clearLMTO();
         ...
         this.mouseout =
	         function(e){
 	            RichFaces.SipMenu.MouseIn = false;
		        if (!RichFaces.SipMenu.selectOpen) {
			         RichFaces.SipMenu.Layers.setLMTO(this.hideDelay);
		        }
		        
		        var unhighlight = this.shouldHighlightParent() && !this.isWithin(e);
		        if(this.onEvt == 'click') {
		        	unhighlight = unhighlight && !RichFaces.SipMenu.Layers.isVisible(this.id); 
		        } 
 		...
 		var binding = new RichFaces.SipMenu.Layer.Binding (
 				this.id,
 				"mouseover",
 				this.mouseover);
 		...
 		binding.refresh();
 		binding = new RichFaces.SipMenu.Layer.Binding (
 				this.id,
 				"mouseout",
 				this.mouseout);
 		this.bindings.push(binding);
        ...
        for(i=0; i<arrayinp.length; i++){
				    ...
				    //var MouseoverInInputb = RichFaces.SipMenu.Layer.MouseoverInInput.bindAsEventListener(this);
				    var MouseoverInInputb = this.MouseoverInInput.bindAsEventListener(this);
                    //var MouseoutInInputb = RichFaces.SipMenu.Layer.MouseoutInInput.bindAsEventListener(this);
					...
					//var OnKeyPressb = RichFaces.SipMenu.Layer.OnKeyPress.bindAsEventListener(this);
...
/*		if(window.A4J && A4J.AJAX ){
	...
	getLabel : function() {
		return RichFaces.SipMenu.Layers.layers[this.layer.id].layer.parentNode.parentNode;
	},
	
	highlightLabel: function() {
		var label1 = $(this.getLabel());
		RichFaces.SipMenu.Items.replaceClasses(label1, 
	...
	getParentLayer: function() {
		return this.level > 0 ? RichFaces.SipMenu.Layers.layers[(RichFaces.SipMenu.Layers.father[this.id])] : null;
	},
	...
	isWithin : function(event){
     ...
     openSelect:  function(event){
	       RichFaces.SipMenu.selectOpen = true;
     ...
     closeSelect: function(event){
       ...
       if (RichFaces.SipMenu.MouseIn == false){
	     RichFaces.SipMenu.Layers.setLMTO(this.hideDelay);
	   }
     },
	...
	showMe: function(e){
   		...
   		RichFaces.SipMenu.Layers.showMenuLayer(this.id, e, this.delay);
   		RichFaces.SipMenu.Layers.levels[this.level] = this;
//		if (this.eventOnOpen) this.eventOnOpen();
	},
	closeSiblings: function(e){
		//LOG.a4j_debug('closeASiblins ' + this.id +' ' +this.level);
		if(RichFaces.SipMenu.Layers.levels[this.level] && RichFaces.SipMenu.Layers.levels[this.level].id != this.id){
   			for(var i = this.level; i < RichFaces.SipMenu.Layers.levels.length; i++){
   				if(RichFaces.SipMenu.Layers.levels[i]) {
   					RichFaces.SipMenu.Layers.levels[i].hideMe();
   					//RichFaces.SipMenu.Layers.levels[i] = '';
   				}
   			}
		}
	},
...
//		if(!item.childMenu){
			...
			for(var i = this.level + (!item.childMenu?1:2); i < RichFaces.SipMenu.Layers.levels.length; i++){
				if(RichFaces.SipMenu.Layers.levels[i]) {
					//LOG.a4j_debug('hide ' +RichFaces.SipMenu.Layers.levels[i]);
					RichFaces.SipMenu.Layers.levels[i].hideMe();
				}
			}
//		}
	...
	addItem: function(itemId, options) {
		var dis = this;
		var item = new RichFaces.SipMenu.Item(itemId, this, options || {});
		//item.menu = this;
		this.items[itemId] = item;
		return this;
	},
	hideMe: function(e){
		RichFaces.SipMenu.Layers.clearPopUpTO();
		RichFaces.SipMenu.Layers.levels[this.level] = null;
		RichFaces.SipMenu.Layers.LMPopUpL(this.id, false,e);
//		if (this.eventOnClose) this.eventOnClose();
	},
	
	isGroupItem: function(target) {
		var item = $(target);
		return (item.hasClassName('rich-menu-group') || item.up('.rich-menu-group') ? true: false);
	},
...
//		if($(topLevel)){  CH-1518
			var menuOn = function(e) {
				RichFaces.SipMenu.Layers.showDropDownLayer(this.id, topLevel, e,this.delay);
			};
			
			var mouseover = function(e) {
                if (!options.disabled && !RichFaces.SipMenu.isWithin(e, $(topLevel))) {
                	this.highlightLabel();
                }
			};
			...
			var blur = function(e){
			/*	
				if(RichFaces.SipMenu.onMenu && !RichFaces.SipMenu.MouseIn) {
					RichFaces.SipMenu.onMenu = false;
				}
				
				if(!RichFaces.SipMenu.onMenu) { 
					RichFaces.SipMenu.Layers.setLMTO(this.delay);
					RichFaces.SipMenu.Layers.clearPopUpTO();
			...
			var mousedown = function(e) {
				var onMenu = this.isGroupItem(Event.element(e));
				RichFaces.SipMenu.onMenu = onMenu;
			...
			var closemenuout = function(e){
				 if (!options.disabled && !RichFaces.SipMenu.isWithin(e, $(topLevel)) && !RichFaces.SipMenu.selectOpen) {
		               	this.unHighlightLabel();
				 }
			}
			
			var menuclick = function(e){
				if(RichFaces.SipMenu.Layers.isVisible(this.id) && !RichFaces.SipMenu.onMenu) {
					RichFaces.SipMenu.Layers.setLMTO(this.delay);
					RichFaces.SipMenu.Layers.clearPopUpTO();
			...
			var addBinding = function(elementId, eventName, handler) {
		 		var binding = new RichFaces.SipMenu.Layer.Binding(elementId, eventName, handler);
		 		this.bindings.push(binding);
		 		binding.refresh();
			}.bind(this);

			if(onEvt && (onEvt == 'onclick')) {
				onEvt = this.eventJsToPrototype(onEvt);
				addBinding(topLevel, onEvt, function(e) {
					menuclick.call(this, e);
				}.bindAsEventListener(this));
				
				
				addBinding(topLevel,'blur',function(e){
					blur.call(this, e);
				}.bindAsEventListener(this));
				
				addBinding(topLevel,'mouseout',function(e){
					closemenuout.call(this, e);
				}.bindAsEventListener(this));
				
				addBinding(topLevel,'mousedown',function(e){
	 		...
	 		RichFaces.SipMenu.Layers.horizontals[this.id] = topLevel;
//		}
		return this;
	},
	...
	asSubMenu: function(parentv, refLayerName, evtName, options){
		...
		this.level = RichFaces.SipMenu.Layers.layers[parentv].level + 1;
   		RichFaces.SipMenu.Layers.father[this.id] = parentv;
   		if(!refLayerName){
   			refLayerName = parentv;
   		}
   		var refLayer = $(refLayerName);
   		this.refItem = RichFaces.SipMenu.Layers.layers[parentv].items[refLayerName];
   		this.refItem.childMenu = this;
 		var binding = new RichFaces.SipMenu.Layer.Binding(refLayerName, evtName,	this.showMe.bindAsEventListener(this));
 		this.bindings.push(binding);
 		binding.refresh();
        ...
        var menuLayer=this;
		while (menuLayer.level > 0) {
					menuLayer = RichFaces.SipMenu.Layers.layers[(RichFaces.SipMenu.Layers.father[menuLayer.id])];
		}
...
RichFaces.SipMenu.Layer.Binding = Class.create();
RichFaces.SipMenu.Layer.Binding.prototype = {
...
RichFaces.SipMenu.isWithin = function (event, element) {
	var within = false;
	Event.extend(event);
...
RichFaces.SipMenu.Item = Class.create({
	...
	onclick: function(e){
		if (this.options.closeOnClick) {
 		...
 		if(!this.options.disabled) {
			RichFaces.SipMenu.Items.onmouseout(this);
 		}
	},
	getElement: function() {
		return $(this.id);
	...
	onmouseover : function(event) {
		...
		if (RichFaces.SipMenu.isWithin(event, element)) {
			return;
		}		
		...
		RichFaces.SipMenu.Items.onmouseover(this);
	},
...
RichFaces.SipMenu.findMenuItem = function (itemId) {
	var layer;
	var menuItem = null;
	for (var id in RichFaces.SipMenu.Layers.layers) {
		layer = RichFaces.SipMenu.Layers.layers[id];
...
RichFaces.SipMenu.updateItem = function (event, element, attr) {
	var menuItem = RichFaces.SipMenu.findMenuItem(element.id);
...
RichFaces.SipMenu.submitForm = function (event, element, params, target, attr) {
	RichFaces.SipMenu.updateItem(event, element, attr);
...
RichFaces.SipMenu.groupMouseOut = function(event, element, menuGroupClass, menuGroupStyle) {
	if (RichFaces.SipMenu.isWithin(event, element)) {
		return; 
	}
...
RichFaces.SipMenu.itemMouseOut = function(event, element, menuItemClass, menuItemStyle, iconClass) {
	if (RichFaces.SipMenu.isWithin(event, element)) {
		return;
	}
...
RichFaces.SipMenu.groupMouseOver = function(event, element, menuGroupHoverClass, menuGroupStyle) {
	if (RichFaces.SipMenu.isWithin(event, element)) {
		return;
	}
...
RichFaces.SipMenu.itemMouseOver = function(event, element, menuItemHoverClass, menuItemStyle, iconClass) {
	if (RichFaces.SipMenu.isWithin(event, element)) {
		return;
	}
