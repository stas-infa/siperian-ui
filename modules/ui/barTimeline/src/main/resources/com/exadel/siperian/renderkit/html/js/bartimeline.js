...
BarTimeline = Class.create({
	...
	onFlashInit: function (){
			...
			if (this.flashComponent)
			{
				...
				if (xmlStringEl) {
					this.flashComponent.setXmlString(unescape(xmlStringEl));
				}
			}
			...
			Utils.execOnLoad(
					...
					function () { return (this.flashComponent && this.flashComponent.setVisibleInterval && typeof(this.flashComponent.setVisibleInterval) == 'function'); }.bind(this), 200);
	},
	...
	onDateRangeChanged : function(viewFromDateNumber, viewToDateNumber){
		...
		this.callEventHandler(this.eventHandlers["onzoomerchange"]);
