...
public abstract class UITab extends AjaxActionComponent implements AjaxComponent, AjaxSource, ActionSource, TabBase, AjaxChildrenEncoder, NamingContainer{
    ...
    public void broadcast(FacesEvent event) throws AbortProcessingException {
    	FacesContext context = getFacesContext();
       	if (event instanceof SwitchablePanelSwitchEvent) {
    		...
    		getPane().updateValue(context, newValue);
    ...
    public void encodeAjaxChild(FacesContext context, String path, Set<String> ids, Set<String> renderedAreas)
        ...
        String currentPath = path + getId() + ":";  
       	UIComponent labelFacet = getFacet("label");
       	if (labelFacet != null) {
       		childrenRenderer.encodeAjaxComponent(context, labelFacet, currentPath, ids, renderedAreas);
       	}
        
       	UIComponent infoImageFacet = getFacet("infoImage");
       	if (infoImageFacet != null) {
       		childrenRenderer.encodeAjaxComponent(context, infoImageFacet, currentPath, ids, renderedAreas);
       	}	
    }
