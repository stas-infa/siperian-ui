...
public class TabRendererBase extends AjaxChildrenRenderer implements TabEncoder { 
    ...
    public boolean isTabRendered(FacesContext context, UITab tab) {
    	return tab.getPane().isTabRendered(tab);
    }
...
//    public String getTabDisplay(FacesContext context,  UIComponent tab) {
//    	TabBase base = (TabBase)tab;
//       	UITabPanel panel = base.getPane();
//        if (panel.isTabSwitched()) {
//            return "display: none;";
//        }else {
//        	return panel.isTabRendered(base) ? "" : "display: none;";  
//        }
//    }
    ...
    protected boolean shouldRenderTabWithHeader(TabBase tab) {
        return tab.getPane().getNewTabNames() != null && tab.getPane().getNewTabNames().contains(tab.getName());
    }
    ...
    public String getTabDisplay(FacesContext context, UIComponent component) {
    	UITab tab = (UITab)component;
    	UITabPanel pane = tab.getPane();
    	if (!pane.isTabSwitched()) {
        	if (tab.getPane().isTabRendered(tab)) {
        		return "";
        	}
    	}
      	return "display: none;";
