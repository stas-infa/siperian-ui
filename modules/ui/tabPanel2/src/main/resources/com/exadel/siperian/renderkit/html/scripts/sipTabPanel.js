...
Object.extend(TabPanel.prototype, {
	...
	initialize: function(id, containerId, selectedTabName, maxTabWidth, ontabchange, ontabclosed, tabsInfo, type, addNewTabScript, highlightStyleClass, tabListHeight) {
		...
		this.tabListRightDiv = $(this.id + ':tabListRight');
		this.tabListDiv = $(this.id + ':tabListDiv');
		this.tabListHeaderClass = this.tabListHeaderDiv.className;
	...
	calculateMaxLabelWidth: function(tab) {
		...
		this.maxLabelWidth = maxTabWidth - Richfaces.getComputedStyleSize(left, 'padding-left') - Richfaces.getComputedStyleSize(right, 'width') - Richfaces.getComputedStyleSize(label, 'padding-right');
	...
	_initScrolling: function () {
		if (this.scroller.isScrollNeed()) {
			...
			if (this.tabsContainer && this.tabsContainer.firstChild) {
				...
				this.tabsContainer.firstChild.style.left = this.left.getWidth() + "px";
			}
			this.onleft(false);
			this.onright(true);
		}else {
			...
			if (this.tabsContainer && this.tabsContainer.firstChild) {
				...
				this.tabsContainer.firstChild.style.left = "0px";
			}
			if (!this.isDynamic()) {
	...
	getScrollWidth: function () {
		...
		return this.tabsContainer.offsetWidth - marginLeft - this.tabListHeaderDiv.getWidth() - this.right.getWidth() + 2 - this.left.getWidth(); 
	},
	...
	adjustTabWidth: function (tab) {
		var label = this.getLabelDiv(tab);
		var isActive = TabPanel.isTabActive(tab);
		this.cutOffLabel(label, this.getLabelWidth(label, isActive));
	},
	...
	getLabelWidth: function (label, isActive){
		var width = isActive ? label.origWidthActive : label.origWidthInActive;
		if (!width){
			if (label.cutOff){
				this.setTextContent(label, label.origText);
			}
			if (isActive){
				label.origWidthActive = label.scrollWidth;
				width = label.origWidthActive;
			} else {
				label.origWidthInActive = label.scrollWidth;
				width = label.origWidthInActive;
			}
		}
		return width;
	},
	...
	cutOffLabel: function (label, origWidth) {
		...
		this.setTextContent(label, text);
		
		this.setActualLabelWidth(label, label.cutOff ? this.maxLabelWidth : origWidth);
	},
	
	setTextContent: function (label, text){
		if (label.innerHTML) {
			var currentText = (label.textContent || label.innerText);
			var c = this.findTextContentHolder(label, currentText);
			if (c && text) {
				c.innerHTML = text;
			}
		}
	...
	initTabListPosition: function () {
		...
		if(tabListHeight == 0) {
			tabListHeight = this.tabListContentDiv.scrollHeight;
		}
		...
		this.tabListLeftDiv.style.height = height;
	   	this.tabListRightDiv.style.height = height;
	},
	...
	showTabList : function (e) {
		this.tabListHeaderDiv.className = this.tabListHeaderClass + 'Clicked';
	...
	hideTabList : function (e) {
		if (e){
			Event.stop(e);
			var list = this.tabListDiv;
			if (this.isWithin(e, list)){
				return ;
			}
		}
		
		this.tabListHeaderDiv.className = this.tabListHeaderClass;
	...
	tabListItemMouseOutListener: function(e){
		...
		if(item && item.tab){
			...
			if (!TabPanel.isTabActive(tab)){
				...
				item.className = "tabListItemActive";
			}
		}
