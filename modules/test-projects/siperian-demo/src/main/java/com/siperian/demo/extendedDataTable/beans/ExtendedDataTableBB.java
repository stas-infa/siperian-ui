...
import com.siperian.demo.extendedDataTable.model.impl.DemoPatient;
import com.siperian.demo.extendedDataTable.model.impl.DemoPatientProvider;
...
public class ExtendedDataTableBB { 
	...
	public boolean filterFirstName(Object current){
	    boolean rst = false;
	...
	public boolean filterLastName(Object current) {
        boolean rst = false;

        if (filterLastNameValue == null || filterLastNameValue.length() == 0) {
            rst = true;
        } else if (current instanceof DemoPatient) {
            DemoPatient currentP = (DemoPatient) current;

            if (currentP.getLastName().toLowerCase().startsWith(this.filterLastNameValue.toLowerCase())) {
                rst = true;
            }
        }
    ...
    public void setSecondSelected() {
		selection.clear();
		selection.addKey(new Integer(1));
	}
