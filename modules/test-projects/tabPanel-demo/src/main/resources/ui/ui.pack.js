...
return false
},handleOnCollapse:function(){if(!this.getComponent().isTimeEditorLayoutCreated&&!this.getComponent().isDateEditorLayoutCreated){this.destroy()
}}});
...
},correctDimensions:function(){if(!this.isMacSafari()){return 
}this.correctDimension(this.getContentElement());
this.correctDimension($(this.div));
this.correctDimension($(this.cursorDiv))
},correctDimension:function(el){el.style.width=this.correctSize(el.style.width);
el.style.height=this.correctSize(el.style.height)
},correctSize:function(v){var val=v;
if(val){val=parseInt(val.replace("px",""));
if(val%2!=0){val=(val+1)+"px";
return val+""
}}return v
},isMacSafari:function(){return(Prototype.Browser.WebKit&&navigator.platform.indexOf("Mac")!=-1)
...
},endDrag:function(border){this.correctDimensions();
for(var k=0;
k<this.borders.length;
...
}if(!this.eventKeyPressAjaxListener){this.eventKeyPressAjaxListener=new A4J.AJAX.Listener(function(req,event,data){if(!$(this.prefix+":n")){Event.stopObserving(document,"keydown",this.eventKeyPress)
}}.bind(this));
...
}}}},removeListeners:function(){if(this.eventKeyPressAjaxListener){A4J.AJAX.removeListener(this.eventKeyPressAjaxListener)
...
this.combobox=null
},addChar:function(keyChar){this.virtualValue=this.virtualValue+String.fromCharCode(keyChar);
LOG.debug("addChar - "+keyChar+",result string ="+this.virtualValue)
},removeChar:function(){if(this.virtualValue){this.virtualValue=this.field.value.substring(0,this.virtualValue.length-1)
}LOG.debug("removeChar - ,result string ="+this.virtualValue)
},setVirtualvalue:function(newValue){this.virtualValue=newValue
...
if(!this.comboList.activeItem){this.comboList.activeItem=this.comboList.selectedItem
}this.setValue(true);
...
}}else{if(this.isSiperianMode){LOG.debug("fieldKeyDownHandler - KEY_DOWN - siperianMode");
this.comboList.moveNotActiveItem(event)
}}break;
...
if(this.isSiperianMode){var value=this.comboList.defaultValue2;
if(value&&value.length>0){this.field.value=value;
...
},fieldFocusHandler:function(){this.comboList.defaultValue2=this.field.value;
this.doActive();
...
if(RichFaces.SipComboBox.SPECIAL_KEYS.indexOf(event.keyCode)==-1){if(event.keyCode!=8){this.addChar(event.keyCode)
}else{if(event.keyCode==8){if(this.virtualValue.length==0){this.field.value="";
this.comboList.selectedItem=null;
this.comboList.createDefaultList();
this.comboList.showWithDelay();
return 
}}}if(this.filterNewValue){LOG.debug("DataUpdating "+this.virtualValue);
var allFoundLength=this.comboList.dataFilter(this.virtualValue);
if(allFoundLength>0&&this.comboList.getItems()&&this.comboList.getItems().length!=0){LOG.debug("dataUpdating list value= "+this.comboList.getItems().length);
...
if(item){if(!this.comboList.visible()){this.comboList.showWithDelay()
...
RichFaces.SipComboBox.SPECIAL_KEYS=[Event.KEY_RETURN,Event.KEY_UP,Event.KEY_DOWN,Event.KEY_RIGHT,Event.KEY_LEFT,Event.KEY_ESC,Event.KEY_TAB,16]
...
}}}else{if(this.selectedItem==null&&curItems!=null&&curItems.length!=0){this.selectedItem=this.createSelectedItemAsDomElement(curItems[0]);
...
}}},createSelectedItemAsDomElement:function(value){var selectedItem=document.createElement("span");
...
}else{if(this.isSiperianMode){var curItems=this.getItems();
LOG.debug("moveActiveItem - KEY_DOWN,curItems.length = "+curItems.length);
if(curItems!=null&&curItems.length!=0){this.doActiveItem(curItems[0]);
this.scrollingUpToItem(curItems[0])
...
}if(itText.substr(0,text.length).toLowerCase()==text.toLowerCase()){items.push(this.crateHighlightItem(itText,text,this.classes.item.normal))
}}return items
...
if(itText.substr(0,substr.length).toLowerCase()==substr.toLowerCase()){item.innerHTML=itText;
...
},crateHighlightItem:function(text,highligh,className){var escapedText=text.escapeHTML();
...
}options["oncomplete"]=function(request,event,data){if(!data){return 
...
if(content){content.noloaded=true
}options["oncomplete"]=function(request,event,data){content=$(env.componentId);
if(content&&content.noloaded){return 
...
}if(!$(tabId)){var content=document.createElement("div");
...
this.switchModeAjaxListener=new A4J.AJAX.Listener(function(req,event,data){if(!$(this.id)&&this.eventKeyPress){Event.stopObserving(document,"keydown",this.eventKeyPress)
}}.bind(this));
...
if(this.switchModeAjaxListener){A4J.AJAX.removeListener(this.switchModeAjaxListener)
}},processPreventLostFocus:function(){this.inFocus=true;
this.preventLostFocus=true
},processLostFocus:function(){if(!this.preventLostFocus){this.lostFocus();
this.finishSwitchMode()
